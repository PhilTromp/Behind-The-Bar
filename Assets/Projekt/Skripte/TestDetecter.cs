...
public class TestDetecter : MonoBehaviour
{
    public string currentDrink;
    public TabletNavigation tabletNavigation;
    ...
    void Start()
    {
        // Falls kein AudioSource an das Objekt angehängt wurde, füge es automatisch hinzu
        if (audioSource == null)
        {
    ...
    void Update()
    {
        if (!string.IsNullOrEmpty(tabletNavigation?.selectedDrink))
        {
            currentDrink = tabletNavigation.selectedDrink;
        }
...
private IEnumerator PlaySoundAfterDelay(float delay)
    ...
    {
        // wird überpüft ob ein Getränk vom Tablet ausgewählt wurde 
